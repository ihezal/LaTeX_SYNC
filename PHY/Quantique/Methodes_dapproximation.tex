\section{Méthodes d’approximation}
    ...
    L’idée de base de la méthode constiste à considérer les vecteurs propres $\ket{\psi(\lambda)}$ et les valeurs propres $E(\lambda)$ de l’hamiltonien $\hat{H}(\lambda) = \hat{H}_0 + \lambda \hat{H}_1$ et à effectuer un développement limité en fonction de $\lambda$ lorsque $\lambda \to 0$. On a ainsi 
    \[ \ket{\psi(\lambda)} = \ket{\psi^{(0)}} + \lambda \ket{\psi^{(1)}} + \cdots \] 
    et 
    \[ E(\lambda) = E^{(0)} + \lambda E^{(1)} + \cdots \]   
    L’équation aux valeurs propres s’écrit alors 
    \begin{align*}
        \left(\hat{H}_0 + \lambda \hat{H}_1\right) \underset{\left(\ket{\psi^{(0)}} + \lambda \ket{\psi^{(1)}} + \cdots\right)}{\ket{\psi(\lambda)}} &= \underset{\left(E^{(0)} + \lambda E^{(1)} + \cdots\right)}{E(\lambda)} \underset{\left(\ket{\psi^{(0)}} + \lambda \ket{\psi^{(1)}} + \cdots\right)}{\ket{\psi(\lambda)}} \\
    \end{align*}
    ...
    \begin{align*}
        ...
        \bra{m} \hat{H}_1 \ket{n} &\overset{\Updownarrow}{=} (E_n - E_m) \spr{m}{\psi^{(1)}} + E^{(1)} \spr{m}{n}
    \end{align*}
    ...
    \[ E \approx E_n + \delta E_n^{(1)} = \bra{n} \hat{H}_0 \ket{n} + \bra{n} \hat{W} \ket{n} = \bra{n} \hat{H} \ket{n} \]   
    ...
    Remarquons que pour tout état $\ket{\psi}$ de l’espace de Hilbert considéré, 
