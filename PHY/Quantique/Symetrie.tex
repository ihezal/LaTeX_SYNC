\section{Symétries en physique quantique}
    ...
    Étudier les symétries d’un système physique est donc judicieux pour trouver des grandeurs conservées. Il est intéressant de remarquer que pour toute isométrie $\hat{R}$ d’un espace de Hilbert $\mathcal{E}_H$, la contrainte $\spr{\psi_1}{\psi_2} = \bra{\psi_1} \hat{R}^{\dagger} \hat{R} \ket{\psi_2}$ pour toutes fonctions d’ondes $\ket{\psi_1}, \ket{\psi_2} \in \mathcal{E}_H$ implique que 
    ...
    \begin{omed}{Commutation de $\hat{R}$ et $\hat{H}$}
        Pour tout système invariant par une isométrie $\mathcal{R}$, l’hamiltonien $\hat{H}$ commute avec l’opérateur $\hat{R}$ représentant cette isométrie dans l’espace de Hilbert, \textit{i.e.}
    ...
    On considère un groupe de symétrie continue, que l’on admettra qu’il peut être caractérisé par un paramètre $a$. On admet aussi que lorsque c’est un groupe de Lie, on peut différencier l’opérateur $\hat{R}_a$ par rapport à $a$. 
    
    \begin{omed}{Générateur infinitésimal}
        Soit $\hat{R}_a$ une isométrie dépendant d’un paramètre $a$. On appelle \textbf{générateur infinitésimal} l’opérateur $\hat{G}$ défini par 
        \[ \hat{R}_{da} = \hat{I} - \frac{i}{\hbar} \hat{G}da \]   
    \end{omed}
    
    Pour satisfaire l’unitarité de $\hat{R}_{da}$, il faut et il suffit que $\hat{G} = \hat{G}^{\dagger}$, \textit{i.e.} que $\hat{G}$ soit un opérateur \textit{auto-adjoint} (\textit{hermitien}). On peut donc lui associer une grandeur physique observable. Par définition de $\hat{G}$, on a 
    ...
    Avec cette nouvelle définition, on peut réinterpréter les rôles de $\hat{U}$ et $\hat{H}$. En effet, en étudiant l’évolution d’un ket entre $t$ et $t + dt$, on a que 
    \[ \ket{\psi(t + dt)} = \hat{U}(t + dt, t) \ket{\psi(t)} \]   
    Or on sait aussi, d’après l’équation de Schrödinger que 
    \begin{align*}
        \ket{\psi(t + dt)}
        &= \ket{\psi(t)} + dt \frac{\text{d}\ket{\psi(t)} }{\text{d}t}  + O\left(dt^2\right) \\
        &= \left[\hat{I} - \frac{i}{\hbar} \hat{H}(t) dt\right] \ket{\psi(t)} + O\left(dt^2\right)
    \end{align*} 
    $\hat{H}$ est donc le générateur infinitésimal de l’évolution temporelle, et on retrouve l’unitarité de $\hat{U}$.

    \subsection{Parité}

    On note $\hat{\Pi}$ l’opérateur parité, dont l’action sur une fonction d’onde $\psi(x)$ de $\mathcal{L}^2(\mathbf{R})$ est,
    \[ \left(\hat{\Pi}_x\psi\right)(x) = \psi(-x) \quad \text{i.e.} \quad \bra{x} \hat{\Pi}_x \ket{\psi} = \spr{-x}{\psi} \]
    On a ainsi $\bra{x} \hat{\Pi}_x = \bra{-x}$ et donc, par unitarité $\hat{\Pi}_x \ket{x} = \ket{-x}$, ce que l’on aurait pu prendre comme point de départ.
    ...
    Considérons un système inviariant par \textit{toute translation}, quelle que soit sa direction où son amplitude. Un tel système est nécessairement homogène. 
    ...
    Les observables $\hat{p}_{x,y,z}$ sont donc les générateurs infinitésimaux du groupe des translations.
        ...
        \subsubsection{Nouvelle définition de l’observable impulsion}
    ...
    Mais la nouvelle définition a l’avantage d’être plus générale que cette dernière équation. Dans un système constitué de $N$ particules, l’opérateur translation s’écrit dans l’espace produit tensoriel $\mathcal{E}_H = \mathcal{E}_1 \otimes \cdots \otimes \mathcal{E}_N$ sous la forme 
    \begin{align*}
        \hat{T}_{d\vec{a}} = \hat{T}_{d\vec{a}}^{(1)} \otimes \cdots \otimes \hat{T}_{d\vec{a}}^{(N)} 
