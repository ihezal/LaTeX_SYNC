\section{Moment cinétique orbital}
    ...
    \subsection{Définition}
        ...
        On pourra donc appliquer au moment cinétique orbital les résultats établis plus haut en matière générale pour tout observable de type moment cinétique. Une des principales différences du moment cinétique orbital est qu’il possède un homologue classique contrairement au moment cinétique intrinsèque.
    ...
    \subsection{Théorie générale du moment cinétique orbital}
        ...
        \begin{align*}
            \left(-\frac{\partial }{\partial \theta} + i \text{cotan}(\theta) \frac{\partial }{\partial \varphi} \right) F(\theta) e^{- i \ell \varphi} &= 0 \\
            \frac{\text{d}F}{\text{d}\theta} &\overset{\Updownarrow}{=} \ell \text{cotan}(\theta) F(\theta) \\
            \frac{\text{d}F}{F} &\overset{\Updownarrow}{=} \ell \text{cotan}(\theta) \text{d}\theta \\
        \end{align*}
        ...
        \begin{itemize}
            \item \textbf{Cas $\ell = 0$ :} $Y_{0,0}(\theta, \varphi) = F_{0,0}(\theta) = c_0$. La normalisation implique que 
            \[ Y_{0,0}(\theta, \varphi) = \frac{1}{\sqrt{4\pi}} \]   
            \item \textbf{Cas $\ell = 1$ :} Avec la formule trouvée précédemment,
            \[ Y_{1, -1}(\theta, \varphi) = \sqrt{\frac{3}{8\pi}} e^{-\varphi} \sin(\theta) \]   
            En appliquant $\hat{L}_+$ à $Y_{1,-1}(\theta,\varphi)$, on trouve 
            \[ Y_{1,0}(\theta, \varphi) = \sqrt{\frac{3}{4\pi}} \cos(\theta) \]   
            La relation de conjugaison nous donne 
        \end{itemize}
