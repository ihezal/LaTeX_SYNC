\section{Outils mathématiques}
    ...
    \subsection{Notation de Dirac}
        ...
        On se place dans l’espace de Hilbert des états $\mathcal{E}_H$. On désigne par \textbf{ket} et on note $\ket{\psi}$ tout vecteur de $\mathcal{E}_H$. On désigne par \textbf{bra} et on note $\bra{\phi}$ tout vecteur de $\mathcal{E}_H^*$ qui est l’unique fonctionnelle linéaire (d’après le théorème de représentation de Riesz) associée $\ket{\phi}$. :
        \[ \bra{\phi} : \ket{\psi} \in \mathcal{E}_H \mapsto \spr{\phi}{\psi} \]

        \subsubsection{Opérateurs linéaires}

        Un opérateur linéaire $\hat{A} : \mathcal{E}_H \to \mathcal{E}_H$ est une fonction linéaire de $\mathcal{E}_H$ dans lui-même. Si on considère une base $\left\{\ket{\psi_n}\right\}$ de $\mathcal{E}_H$, on peut définir la matrice de l’opérateur dans cette base, qui aura pour coefficients 
        ...
        L’opérateur le plus commun est l’identité, qui peut s’écrire dans une base orthonormale de $\mathcal{E}_H$ comme 
        ...
        \begin{omed}{Théorème spectral}
            ...
            où les $a_n \in \text{sp}(\hat{A})$ sont des réels.
        \end{omed}

    \subsection{Transformation de Fourier}

        Considérons le cas d’une particule se déplaçant sur un axe qui décrit sa position. On sait que l’espace des états $\mathcal{E}_H$ est homéomorphe à $\mathcal{L}^2(\mathbf{R})$ et on se place dans cette sous-section dans le formalisme de $\mathcal{L}^2(\mathbf{R})$. 
        ...
        Pour exprimer le contenu de cette intégrale, il nous faut la valeur de $\spr{p}{x}$. On sait que $\hat{p} = \frac{\hbar}{i} \frac{\text{d}}{\text{d}x}$, donc 
        \begin{align*}
            p\spr{x}{p} 
            &= \bra{x} \hat{p} \ket{p} \\
            &= \frac{\hbar}{i} \frac{\text{d}}{\text{d}x} \spr{x}{p} \\ 
        \end{align*}
        Par résolution de l’équation différentielle, on obtient $\spr{x}{p} = C(p) \exp\left(\frac{ipx}{\hbar}\right)$, puis la normalisation implique que 
        \[ \spr{x}{p} = \frac{e^{\frac{i p_x x}{\hbar}}}{\sqrt{2\pi \hbar}} \]
        Donc 
